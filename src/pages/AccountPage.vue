            ...
            <div class="row">
              ...
              <div class="col-8">
                ...
                <div class="row">
                  ...
                  >{{ Auth.auth.profile.long_bio }}</div>
                </div>
              </div>
              <div class="col-12">
                <div class="row q-pa-sm">
                  <div class="col-3">
                    <div class="text-h5 text-bold">{{ Auth.auth.lesson_plans_count }}</div>
                    <div class="text-body2 text-grey-4">RPP</div>
                  </div>
                  <q-separator vertical color="grey-4" />
                  <div class="col-4 q-ml-lg">
                    <div class="text-h5 text-bold">{{ Auth.auth.lesson_plans_my_likes_count }}</div>
                    <div class="text-body2 text-grey-4">Menyukai</div>
                  </div>

                  <div class="col-4">
                    <div class="text-h5 text-bold">{{ Auth.auth.lesson_plans_likes_count }}</div>
            ...
            <div class="q-pa-sm q-px-md">
              <div class="row justify-between">
                ...
                <q-btn
                  ...
                  rounded
                  dense
                  style="background-color:#564395"
                  @click="$router.push(`/MoreLessonPlans/${Auth.auth.id}`)"
                >Lihat Semua</q-btn>
              </div>
            </div>
            <div class="q-ml-xl q-mt-md">
              ...
              >
                <div>
                  <q-card
                    class="mycard"
                    @click="$router.push(`/user/lessonplanlist/${Auth.auth.id}/${item.id}`)"
                  >
                    <q-card-section>
                      <q-img
                        :src="item.cover == null ? `https://cdn.qubicle.id/images/2019/02/15/be26ee32-34a8-407a-a712-3a3e61cdaff8.jpg`
                    :`${Setting.storageUrl}/${item.cover.image}`"
                      ></q-img>
                      <div class="text-body1">{{ item.topic }}</div>
                    </q-card-section>
                  </q-card>
                </div>
              </RecycleScroller>
