...
<div>
    <q-item clickable v-ripple v-if="comment != null">
        ...
        <q-item-section top>
            ...
            <q-item-label caption>
                <div class="row">
                    <div class="text-caption text-grey q-pr-md">{{comment.created_at | moment('from','now')}}</div>
                    <div class="text-caption text-grey text-bold" v-show="comment.likes_count" @click="$router.push(`/lessonplan/comment/like/${comment.id}`)">{{comment.likes_count}} suka</div>
                </div>
            </q-item-label>
        </q-item-section>

        <q-item-section side top>
            <q-btn flat size="sm" class="q-mt-sm" round :color="comment.liked_count ? 'red' : null" :icon="comment.liked_count ? 'favorite' : 'favorite_border'" @click="comment.liked_count ? dislike(comment.id) : like(comment.id)" />
        </q-item-section>
    </q-item>
</div>
</template>
