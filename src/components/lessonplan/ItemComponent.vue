        ...
        <div class="row">
          ...
          <div class="col-2 self-center">
            <div class="row justify-end">
              <q-btn flat round dense size="md" icon="more_vert" @click="showMenu()" />
            </div>
          </div>
        </div>
      </q-card-section>
              ...
              >
                <div v-if="lessonplan.ratings_value_count">
                  ...
                  <div class="text-caption text-deep-purple">
                    {{ lessonplan.ratings_value_count / lessonplan.ratings.length }}
                  </div>
                </div>
              </div>
              ...
              v-if="
                Auth.auth.profile.city_id
                  ? lessonplan.user.profile
                    ? Auth.auth.profile.city_id === lessonplan.user.profile.city_id
                    : false
                  : false
              "
              color="deep-purple"
...
export default {
  ...
  computed: {
    ...
    lessonPlanImage: function () {
      if (!this.imageLoaded && this.useDefaultImage) {
        ...
        const image = this.lessonplan.template
          ? this.lessonplan.template.image
          : this.lessonplan.cover.image;
    ...
    isObserverOrIsAuth: function () {
      return this.Auth.auth.profile.city_id
        ? this.lessonplan.user.profile
          ? (this.Auth.auth.profile.city_id === this.lessonplan.user.profile.city_id &&
              this.Auth.auth.role_id == 7) ||
  ...
  created() {
    this.lessonplan.rated_value_count
      ? (this.lessonplan.rated_value_count = parseInt(this.lessonplan.rated_value_count))
      : (this.lessonplan.rated_value_count = 0);

    if (this.lessonplan.user.id == this.Auth.auth.id) {
      this.actions.push({
        label: "Edit",
        icon: "edit",
        color: "deep-purple",
  ...
  methods: {
    ...
    rate() {
      ...
      this.$store.dispatch("LessonPlan/rate", access).then((res) => {
        //
        this.lessonplan.rated_value_count = parseInt(res.data.rated_value_count);
